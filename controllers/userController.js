...
class userController {
    ...
    async handleRegister(req, res) {
        try {
            ...
            return res.status(500).json({ message: 'Lỗi khi đăng ký người dùng mới' });
        }
    }
    ...
    async handleLogin(req, res) {
        try {
            ...
            const refreshToken = jwt.sign({ _id: user._id }, "nguyet", { expiresIn: '30d' })
            const access_token = jwt.sign({ _id: user._id }, "nguyet", { expiresIn: '3h' })
            ...
            await user.save();
            return res.status(200).json({ refresh_token: refreshToken, access_token: access_token, user_id: user._id, role: user.role })
        } catch (err) {
            console.error(err);
    ...
    async sendResetPasswordLink(req, res) {
        ...
        transporter.sendMail(mailOptions, (error, info) => {
            if (error) {
                ...
                res.status(200).json({ message: 'The password reset link has been sent successfully' });
            }
        });
    }
    ...
    async verifyTokenResetPassword(req,res){
